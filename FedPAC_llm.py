...
from rite import collect_paired_lora_params
#from optimizer import SAM
from sam import SAM
from muon import SingleDeviceMuonWithAuxAdam

from optimizer.adamw_L import LayerWiseAdamW
#gc.collect()
...
from optimizer.adam_mini import Adam_mini
#from sophia import SophiaG
...
from optimizer.mars import MARS
...
def SOAP(params, lr, betas, shampoo_beta, weight_decay, precondition_frequency):
    pass
...
class DataWorker(object):
    ...
    def update_Local_Muon(self, weights, E, index, lr):
        ...
        param_groups = [
            dict(params=hidden_weights, use_muon=True,
                 lr=lr*args.alpha, weight_decay=0.01,momentum=0.95),
    ...
    def update_fedavg_soap(self, weights, E, index, lr):
        ...
        self.data_id_loader(index)
        from soap import SOAP
        self.optimizer = SOAP( self.model.parameters(),lr=lr, betas=(.95, .95),eps=1e-6, weight_decay=0, precondition_frequency=10)
        step = 0  # 新增步数计数
        self.loss =0
        for e in range(E):
            for batch in tqdm(self.data_iterator, disable=True):
    ...
    def update_fedavg_Sophia(self, weights, E, index, lr):
        ...
        for name, param in self.model.named_parameters():
            if "classifier" in name or "head" in name:
                   param.requires_grad = True
        self.optimizer  = SophiaG(self.model.parameters(), lr=lr, betas=(0.965, 0.99), rho=0.01, weight_decay=1e-1)
    ...
    def update_FedMuonAC(self, weights, E, index, ps_c,momen_m, lr,step):
        ...
        #        continue
        #    ps_c[k] = ps_c[k].to(device)
        ps_c = {k: v.to(device) for k, v in ps_c.items()}
        ...
        param_groups = [
            dict(params=hidden_weights, use_muon=True,
                 lr=lr * args.alpha * (1 - args.gamma), weight_decay=0.01,momentum=0.98),
            ...
            for batch in tqdm(self.data_iterator, disable=True):
                ...
                with torch.no_grad():
                    for n, p in model.named_parameters():
                        if not p.requires_grad or n not in ps_c:
                            continue
                        p.add_(ps_c[n], alpha=args.gamma * lr / K)
        momen_m={}
        for group in self.optimizer.param_groups:
            for p in group['params']:
                if p.grad is None:
    ...
    def update_Fedsoap(self, weights, E, index, momen_m, momen_v, lr, step):
        ...
        for k in momen_m.keys():
            momen_m[k] = momen_m[k].to(device)
        self.optimizer = SOAP(params= self.model.parameters(),lr=lr, betas=(0.95, 0.95),shampoo_beta=0.95, weight_decay=0.01, precondition_frequency=10)
        for group in self.optimizer.param_groups:
            for p in group['params']:
                if p.grad is None:
                    continue
    ...
    def update_func(self, alg, weights, E, index, lr, ps_c=None, v=None,step=None,shared_state=None):
        self.load_dict()
        if alg in { 'FedCM','FedadamW_CM','FedCM','FedAdamWc'}:
            return self.func_dict.get(alg, None)(weights, E, index, ps_c, lr)
        if alg in {'FedLADA','FedAdamW','FedMuonAC','FedSoap'}:
...
if __name__ == "__main__":
    ...
    for epochidx in range(epoch_s, epoch):
        ...
        if alg in {'FedSoap'}:
            ...
            for i in range(0, n, int(n / args.p)):
                ...
                weights_and_ci = weights_and_ci + [
                    ...
                    zip(workers, index_sel)]
            weights_and_ci = ray.get(weights_and_ci)
            current_weights, m, v = apply_weights_Fedsoap(num_workers, weights_and_ci, model, m)
            model.load_state_dict(current_weights)
            step.add_(args.K)
        ...
        if alg in {'FedLADA','FedAdamW','FedMuonAC'}:
            weights_and_ci = []
            n = int(num_workers * selection)
            for i in range(0, n, int(n / args.p)):
                index_sel = index[i:i + int(n / args.p)]
